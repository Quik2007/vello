...
struct GpuResources {
    ...
    atlas_texture_ids: Vec<TextureId>,
...
impl Programs {
    fn new(
        ...
        let atlas_texture_views = atlas_textures
            ...
            .collect();
        let atlas_texture_ids = (0..*initial_atlas_count)
            .map(|_| TextureId::new())
            .collect();
    ...
    fn maybe_create_atlas_textures(
        ...
        while resources.atlas_textures.len() < required_atlas_count as usize {
            ...
            resources.atlas_texture_ids.push(TextureId::new());
        }
    }
...
impl RendererContext<'_> {
    fn external_texture_bind_group_for_textures(
        ...
        match self.external_texture_bind_groups.entry(bindings) {
            ...
            Entry::Vacant(entry) => {
                let texture_views = bindings.as_array().map(|external_texture_id| {
                    match external_texture_id {
                        Some(external_texture_id) => {
                            ...
                            {
                                self.programs.resources.atlas_texture_views[atlas_index].clone()
                            } else {
                                self.texture_bindings
                                    .get(external_texture_id)
                                    .expect(
